...
async def generate_video(
    ...
    # If character image is provided, prepare it once
    char_img_path = None
    if character_image:
    ...
    for i, line in enumerate(script_lines):
        # Create frame image using character image or default black
        if character_image:
            img = char_img.copy()
        else:
        ...
        draw.text((50, 620), line, font=font, fill='white')  # Draw text near bottom

        img_path = f"static/frame_{uuid.uuid4().hex}.png"
        img.save(img_path)

        # Handle voice file or fallback to silence
        ...
        # Combine image and voice into video clip
